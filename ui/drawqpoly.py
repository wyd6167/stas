...
class InteractivePolygon(QGraphicsPolygonItem):
    ...
    def mousePressEvent(self, event):
        ...
        for i in range(self.polygon().size()):
            if (self.mapToScene(event.pos()) - self.polygon().at(i)).manhattanLength() < 10:
    ...
    def mouseMoveEvent(self, event):
        if self._selected_point_index is not None:
            ...
            polygon.replace(self._selected_point_index, QPointF(new_point))
...
class GripItem(QtWidgets.QGraphicsPathItem):
    ...
    circle.addEllipse(QtCore.QRectF(-10, -10, 2, 2))
    square = QtGui.QPainterPath()
    square.addRect(QtCore.QRectF(-15, -15, 3, 3))
