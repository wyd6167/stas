...
from utils.losses import ContrastiveCenterLoss,CenterLoss
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.Contraloss = ContrastiveCenterLoss(64, 1)
        self.Centerloss = CenterLoss(64, 1)
    def __call__(self, p, targets,feature):  # predictions, targets, model
        ...
        lobj *= self.hyp['obj']
        lcls *= self.hyp['cls']
        lcontra = self.Centerloss(feature, torch.zeros(feature.size(0), device=device, dtype=torch.int64)).unsqueeze(0)

        bs = tobj.shape[0]  # batch size
        loss = lbox + lobj + lcls + lcontra * 0.3
        # loss = lbox + lobj + lcls
        return loss * bs, torch.cat((lbox, lobj, lcls,lcontra, loss)).detach()
...
class ComputeLossOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.Contraloss=ContrastiveCenterLoss(64,1)
        self.Centerloss=CenterLoss(64,1)
    def __call__(self, p, targets, imgs,feature):  # predictions, targets, model

        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        lcontra = torch.zeros(1, device=device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    lcls += self.BCEcls(ps[:, 5:], t)  # BCE
                # lcontra=self.Contraloss(feature,torch.zeros(feature.size(0), device=device,dtype=torch.int64))
        ...
        lcontra = self.Centerloss(feature, torch.zeros(feature.size(0), device=device, dtype=torch.int64)).unsqueeze(0)
        # import numpy as np
        # lcontra = np.expand_dims(lcontra.cpu(),0)
        # lcontra = torch.tensor(lcontra,device=device)
        if self.autobalance:
            self.balance = [x / self.balance[self.ssi] for x in self.balance]
        lbox *= self.hyp['box']
        lobj *= self.hyp['obj']
