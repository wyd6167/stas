...
class CenterLoss(nn.Module):
    ...
    def forward(self, feat, label):
        ...
        batch_size = feat.size()[0]
        expanded_centers = self.centers.index_select(dim=0, index=label).cuda()
        # print(feat.size(),expanded_centers.size())
...
class ContrastiveCenterLoss(nn.Module):
    ...
    def forward(self, feat, label):
        batch_size = feat.size()[0]
        expanded_centers = self.centers.expand(batch_size, -1, -1).cuda()
        expanded_feat = feat.expand(self.num_classes, -1, -1).transpose(1, 0).cuda()
        distance_centers = (expanded_feat - expanded_centers).pow(2).sum(dim=-1).cuda()
