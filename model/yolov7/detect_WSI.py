...
from model.yolov7.utils.plots import plot_one_box
from model.yolov7.utils.torch_utils import select_device, time_synchronized, TracedModel
...
def detect_one(svs_path):
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str,
                        default=r'F:\python\mypy1\stas_system\stas_system\model\yolov7\weights\best.pt',
                        help='model.pt path(s)')
    parser.add_argument('--source', type=str,
                        default='/home/hansheng/wyd/stas_detection/pytorch-deeplab-xception-master/data_new/test_whole_new',
    ...
    # contour_path='/home/hansheng/wyd/stas_detection/pytorch-deeplab-xception-master/data_new/11/'
    contour_path=r'F:\python\mypy1\stas_system\stas_system\result/'

    svs = opensvs(svs_path)


    patch_size = 512
    step = patch_size
    name=svs_path.split('/')[-1].replace('svs','png')
    res_contours=get_res_contours(contour_path + name)
    # cur_slide = s.read(location=[0, 0], scale=8)
    ...
    whole_size_i = svs.height
    whole_size_j = svs.width
    ...
    for i in range(0, whole_size_i, step):
        for j in range(0, whole_size_j, step):
            ...
            cur_slide = svs.read(location=[j, i], scale=1 , size=[patch_size_j,patch_size_i])
            img = cur_slide
...
def detect(save_img=False):
    ...
    svs_pa='E:/显微镜/qiqiangdata/'
    list_slide = os.listdir(svs_pa)
    ...
    for ss in list_slide:
        ...
        cur_slide = s.read(location=[0, 0], scale=16)
        print(cur_slide.shape)
        # resize_ratio = s.level_downsamples[level] / 8
        whole_size_i = cur_slide.shape[0]  # y
        whole_size_j = cur_slide.shape[1]  # x
    ...
    def detect_one(self, svs_path):
        ...
        for i in range(0, whole_size_i, step):
            ...
            for j in range(0, whole_size_j, step):
                # print(i, j)
                if judge_box_in_contour_1(res_contours,
                                          [j, i, j + patch_size, i + patch_size]) == False:
                    continue
                # print(1)
                if i + patch_size_i > whole_size_i:
                    i = whole_size_i - patch_size_i
                if j + patch_size_j > whole_size_j:
                    j = whole_size_j - patch_size_j
                self.jindu = (i + step) / whole_size_i

                if j + 513 > whole_size_j:
                    print("进度:  ", self.jindu)
                    global_var.set_value('进度', self.jindu)
                cur_slide = svs.read(location=[j, i], scale=1, size=[patch_size_j, patch_size_i])
                img = cur_slide
                # img = cur_slide[i:i + patch_size, j:j + patch_size, :]
                img0 = img.copy()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=r'F:\python\mypy1\stas_system\stas_system\model\yolov7\weights\best.pt', help='model.pt path(s)')
    ...
    #     else:
    #         detect()
    svs_path = 'E:/显微镜/qiqiangdata/1637460-G.svs'
    det = Detect()
    det.detect_one(svs_path)
    # detect_one(svs_path)
